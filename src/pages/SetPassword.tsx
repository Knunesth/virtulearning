...
export const SetPassword = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg relative overflow-hidden px-4">
        <div className="max-w-md w-full bg-card/50 backdrop-blur-xl border border-border rounded-3xl p-8 shadow-2xl relative z-10 text-center">
          ...
          <p className="text-muted mb-6">O link de definição de senha é inválido ou está ausente.</p>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Erro ao definir a senha. O link pode estar expirado.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg relative overflow-hidden px-4">
      ...
      <div className="max-w-md w-full bg-card/50 backdrop-blur-xl border border-border rounded-3xl p-8 md:p-10 shadow-2xl relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-white mb-2">Defina sua Senha</h2>
          <p className="text-muted text-sm px-4">
            Sua candidatura foi aprovada! Escolha uma senha segura para acessar sua conta de professor.
          </p>
        </div>
          ...
          <div className="mb-6 p-4 rounded-xl bg-danger/10 border border-danger/20 text-danger text-sm font-medium flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-danger/80">
                Se o link expirou, entre em <a href="mailto:suporte@virtulearning.com" className="underline">contato com o suporte</a>.
              </p>
            </div>
          </div>
        )}
